...
// be its own file at api/cron/expire-reservations.js, folded in here (under
// the GET branch below) purely to stay under that limit. Vercel Cron always
// sends a GET, so method alone is enough to tell it apart from the POST
// action-dispatch admin actions below use.
...
// manual cleanup. Restores stock exactly like a manual release, just tagged
// as "reservation_expired" and actor "system" in the ledger.
async function handleExpireReservationsCron(req, res) {
...
export default async function handler(req, res) {
    ...
    if (req.method !== "POST") {
        return res.status(405).json({ message: "Only GET (cron) or POST (actions) allowed" });
    }
