...
// Triggers the business-initiated WhatsApp shipped/dispatch notification,
// called right after a Shiprocket shipment + AWB are created. Same shared-
// secret auth as the order-confirmation notification above.
export async function sendShippedNotificationWhatsApp({ phone, customerName, orderNumber, courierName, trackingId }) {
  const baseUrl = process.env.WHATSAPP_AGENT_URL;
  const secret = process.env.NOTIFY_API_SECRET;

  if (!baseUrl || !secret) {
    console.warn('⚠️ WHATSAPP_AGENT_URL or NOTIFY_API_SECRET not configured; skipping WhatsApp shipped notification');
    return;
  }
  ...
  try {
    const res = await fetch(`${baseUrl.replace(/\/$/, '')}/api/notify/order-shipped`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-notify-secret': secret,
      },
      body: JSON.stringify({ phone, customerName, orderNumber, courierName, trackingId }),
      signal: controller.signal,
    });

    if (!res.ok) {
      const errorBody = await res.text().catch(() => '');
      console.error(`❌ WhatsApp shipped notification failed (${res.status}):`, errorBody);
      return;
    }

    console.log('✅ WhatsApp shipped notification sent');
  } catch (error) {
    console.error('❌ WhatsApp shipped notification request failed:', error.message);
  } finally {
    clearTimeout(timeout);
  }
}
