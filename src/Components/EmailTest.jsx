  ...
  const testEmail = async () => {
    ...
    try {
      ...
      if (response.ok && responseData.success) {
        setResult(`✅ Test order placed successfully! Email should be sent to ${email}. Check your inbox and spam folder.`);
      } else {
        setResult(`⚠️ API call completed but may have issues. Check console for details. Response: ${JSON.stringify(responseData)}`);
      }
      
    } catch (error) {
      console.error('❌ Test failed:', error);
      setResult(`❌ Test failed: ${error.message}. This might be expected since we're testing in development mode.`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ 
      ...
      {result && (
        <div style={{
          padding: '15px',
          borderRadius: '8px',
          backgroundColor: result.includes('✅') ? '#ecfdf5' : result.includes('💡') ? '#f0f9ff' : '#fef2f2',
          border: `1px solid ${result.includes('✅') ? '#10b981' : result.includes('💡') ? '#3b82f6' : '#ef4444'}`,
          color: result.includes('✅') ? '#065f46' : result.includes('💡') ? '#1e40af' : '#991b1b',
          fontSize: '14px',
          lineHeight: '1.5',
          whiteSpace: 'pre-line'
        }}>
